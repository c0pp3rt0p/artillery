...
function runScenario(script, intermediate, runState) {
  ...
  if (!runState.compiledScenarios) {
    ...
    runState.scenarioEvents.on('response', function(delta, code, uid, request, scenarioName) {
      intermediate.completedRequest();
      intermediate.addLatency(delta);
      intermediate.addCode(code);

      let entry = [Date.now(), uid, delta, code, {url: request.url, method: request.method}, scenarioName];
